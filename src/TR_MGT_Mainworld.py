...
# Define setters, including checks
    ...
    def temperature(self, temperature):
        if temperature not in TR_MGT_Constants.TEMPERATURES:
            logger.error("Temperature value %s not valid, setting to TEMPERATE", temperature)
...
# Initialise the world class        
    ...
    def gen_temperature(self, roll):
        ...
        modifider = 0
        if self.atm <= 1: modifier = 0
        elif self.atm in [2, 3]: modifier = -2
        elif self.atm in [4, 5, 14]: modifier = -1
        elif self.atm in [6, 7]: modifier = 0
        elif self.atm in [8, 9]: modifier = 1
        elif self.atm in [10, 13, 15]: modifier = 2
        elif self.atm in [11, 12]: modifier = 6

        logger.debug("Roll = %s, Modifier = %s", roll, modifier)

        roll += modifier

        if roll <= 2: temperature = "FROZEN"
        elif roll in [3, 4]: temperature = "COLD"
        elif roll in [5, 6, 7, 8, 9]: temperature = "TEMPERATE"
        elif roll in [10, 11]: temperature = "HOT"
        elif roll >= 12: temperature = "BOILING"
    ...
    def gen_hyd(self, roll):
        ...
        if self.siz == 0: x = 0
            ...
            if self.atm not in [13, 15]:
                if self.temperature == "HOT": x -= 2
                elif self.temperature == "BOILING": x -= 6
        if x < 0: x = 0
        if x > 10: x = 10
        logger.debug('Result = %s', x)
        self.hyd = x
    ...
    def gen_starPort(self, roll):
        ...
        if self.pop in [8, 9]: spRoll += 1
        elif self.pop >= 10: spRoll +2
        elif self.pop in [3, 4]: spRoll -= 1
        elif self.pop <= 2: spRoll -= 2 

        if spRoll < 2:  spRoll = 2
        if spRoll > 12:  spRoll = 12

        logger.debug('Result = %s', spRoll)
        logger.info('Mainworld starport type is %s', TR_Constants.STARPORTSTABLE[spRoll])
    ...
    def gen_tlv(self, roll, tlcap):
        ...
        if self.atm == 12: minTL = 10
        elif self.atm == 11: minTL = 9
        elif self.atm in [0, 1, 10, 15]: minTL = 8
        elif self.atm in [2, 3, 13, 14]: minTL = 5
        elif self.atm in [4, 7, 9]: minTL = 3
        else: minTL = 0
    ...
    def gen_bases(self):
        ...
        sBase = False     
        if self.starPort == "A": sBase = (D6Rollx2() >= 10)
        if self.starPort == "B": sBase = (D6Rollx2() >= 9)
        if self.starPort == "C": sBase = (D6Rollx2() >= 8)

        # Pirate bases

        pBase = False
        if self.starPort == "D": pBase = (D6Rollx2() == 12)
        if self.starPort in ["E", "X"]: pBase = (D6Rollx2() >= 10)
    ...
    def gen_travelZone(self, roll, isRandom):
        ...
        if (self.atm >= 10) or (self.gov in [0, 7, 10]) or (self.law == 0) or (self.law >= 9):
        ...
        if isRandom and travelZone == ' ':
            logger.debug('Random travel zone generation is set, generating travel zones')
            x = D6Rollx3()
            logger.debug('Roll = %s', x)
            if x >= 16: travelZone = 'A'
            if x == 18: travelZone = 'R'

        else: logger.debug('Random travel zone generation is not set')
